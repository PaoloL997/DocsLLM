    ...
    if (messageInput) {
        ...
        messageInput.addEventListener('keydown', function(event) {
            if (event.key === 'Enter' && !event.shiftKey) {
                ...
                if (isLoggedIn && activeCollection) sendMessage();
            }
        });
    }
...
function openSourceModal(btnDef) {
    ...
    const ps = (btnDef.page_start != null) ? btnDef.page_start : meta.page_start;
    const pe = (btnDef.page_end != null) ? btnDef.page_end : meta.page_end;
    const namespace = meta.namespace || meta.name || btnDef.name || 'Fonte';

    let headerTitle = namespace;
    if (ps != null && String(ps) !== 'N/A') {
        const s = String(ps);
        const e = (pe != null && String(pe) !== 'N/A') ? String(pe) : null;
        headerTitle += (e && e !== s) ? ` — pag. ${s}-${e}` : ` — pag. ${s}`;
    }
    ...
    const pathVal = (meta.path || meta.source) || btnDef.name || '';
    if (!pathVal) {
        ...
        fetch('/api/check-path/', {
            ...
            body: JSON.stringify({
                path: pathVal,
                type: rawType,
                page_start: ps != null ? ps : null,
                page_end: pe != null ? pe : (ps != null ? ps : null)
            })
        }).then(r => r.json()).then(res => {
            loading.remove();
            if (!res) { content.appendChild(Object.assign(document.createElement('p'), { textContent: 'Anteprima non disponibile.' })); return; }

            if (res.error) {
                content.appendChild(Object.assign(document.createElement('p'), { textContent: 'Errore: ' + res.error }));
            } else if (res.preview != null) {
                const pre = document.createElement('pre');
                pre.className = 'source-preview';
                pre.style.cssText = 'white-space:pre-wrap;max-height:60vh;overflow:auto';
                pre.textContent = res.preview;
                content.appendChild(pre);
            } else if (res.pdf_data_uri) {
                const base64 = res.pdf_data_uri.split(',')[1];
            ...
            } else if (res.url || res.http_url) {
                const iframe = document.createElement('iframe');
                iframe.src = res.url || res.http_url;
                iframe.style.cssText = 'width:100%;height:100%;border:none';
                iframe.title = 'Anteprima PDF';
                content.appendChild(iframe);
                ...
                img.style.cssText = 'max-width:100%;max-height:60vh';
                ...
                content.appendChild(Object.assign(document.createElement('p'), { textContent: 'Anteprima non disponibile.' }));
            }
        }).catch(() => {
